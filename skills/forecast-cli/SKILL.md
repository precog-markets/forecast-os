...
name: forecast-cli
description: >-
  Operate the `forecast` CLI for Polymarket, Precog, and Kalshi.
  Use when installing forecast, discovering or quoting markets, buying or
  managing positions, creating a Precog market, or checking setup, status, or config.
metadata:
  cli: forecast
---
...
| Intent | Load |
| --- | --- |
| Install or verify `forecast` | [config-and-auth.md](references/config-and-auth.md) |
| Config, setup, status, secrets | [config-and-auth.md](references/config-and-auth.md) |
| Flags, predict pairs, create spec, command map | [commands.md](references/commands.md) |
...
Done when the loaded file's **Done when** holds, or you have explained a clear CLI failure (exit codes in [pitfalls.md](references/pitfalls.md)).
