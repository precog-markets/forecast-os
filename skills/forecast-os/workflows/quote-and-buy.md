...
If the binary is missing, load [config-and-auth.md](../references/config-and-auth.md). If short refs or predict pairs fail, load [pitfalls.md](../references/pitfalls.md) and [commands.md](../references/commands.md). Follow the setup loop in [config-and-auth.md](../references/config-and-auth.md) only before `--confirm`.

## Quote or buy

1. `market get` the chosen market with an absolute ref (required before any `OUT:*` predict):

```bash
forecast market get POLYMARKET:EVENT:EVENT_ID --only-outcomes --all --output json --no-input
```

2. Quote with exactly one option pair from [commands.md](../references/commands.md). Precog cannot use `--buy-size` alone (add `--price-limit`, or use `--buy-shares` + `--spend-limit`):
