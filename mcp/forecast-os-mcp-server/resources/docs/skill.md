...
- After wallet creation fields are resolved, use `run_skill_step` with the current `create_market` workflow state so `.forecastos` advances to `await_precog_approval`; reserve direct `create_market` for low-level calls that do not need workflow memory updates. When creation succeeds, immediately set up the host's hourly automation/cron equivalent from the returned `pending_check` object.
