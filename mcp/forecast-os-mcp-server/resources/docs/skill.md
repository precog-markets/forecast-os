...
- Assume every market is `multi_outcome`; do not reduce yes/no-shaped ideas to only `Yes` and `No`.
...
- Do not use only `Yes` / `No` outcomes in normal ForecastOS drafts. For yes/no-shaped prompts, create at least three concrete outcomes. For release/date questions, prefer time buckets such as `Released in 2027`, `Released before 2027`, `Released after 2027`, and `No official release / cancelled`.
...
- For live Precog creation or funding, ask which wallet or wallet/action tool the user wants to use. For creation, offer [Bankr](https://bankr.bot), [Privy](https://www.privy.io/ai), [Base MCP](https://mcp.base.org), another configured wallet/action tool, or the [Precog creation area](https://core.precog.markets/launchpad/); Base Account smart-wallet signatures verified through EIP-1271/ERC-6492 are valid when signed over the canonical Precog typed data and current pending nonce. For funding, [Bankr](https://bankr.bot), [Base MCP](https://mcp.base.org), or another configured wallet/action tool may be used after a prepared unsigned calldata envelope exists; with Base MCP, run `send_calls` first, then sign `FUND_UPCOMING_MARKET` using the post-transaction pending nonce before submitting to Precog. Do not ask for raw wallet addresses or signatures in normal chat. Do not ask which creation venue to use unless the user explicitly introduces a non-Precog venue; explain that ForecastOS can create through Precog and can only read external venues such as Polymarket or Kalshi.
...
- After wallet creation fields are resolved, prefer `publish_approved_market --workflow-id <workflow_id> --wallet-output <wallet-output-json>` so the bridge loads the stored `create_market` workflow and advances `.forecastos` to `await_precog_approval`; use raw `run_skill_step` only when the host already has the exact persisted state object. Reserve direct `create_market` for low-level calls that do not need workflow memory updates. When creation succeeds, immediately set up the host's hourly automation/cron equivalent from the returned `pending_check` object.
