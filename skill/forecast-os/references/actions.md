...
The shipped `config.json` contains public defaults so users can run the skill without setup. ForecastOS core supports Base (`8453`) and Arbitrum (`42161`) through config. If chain/collateral is missing in user input, ask explicitly (for example, `With collateral from which chain?`) and offer defaults `USDC on Base` and `USDC on Arbitrum`. If the user already specifies chain/collateral, respect it by passing `chain_id` and the matching collateral through draft, approval, and create events; ForecastOS resolves the active chain from that workflow context and uses the matching `supported_chains[chain_id]` deployed master and default collateral. For explicit non-default collateral, pass `collateral_address` and `collateral_symbol` (aliases `requested_collateral_address` / `requested_collateral_symbol` on draft input). ForecastOS warns when the address is not in `default_collateral_options`; Precog may reject unsupported tokens at create. Do not edit `.forecastos/config.json` or hand-write workflow files for normal chat creation. Optional `.forecastos/config.local.json` may override defaults for local testing only.

`draft_market` routes through the bundled `run_skill_step` intake path, persists workflow state at `await_approval`, and returns `state.workflow_id`, `state.draft_id`, and `state.draft_hash` for the next approval step. Reuse the full returned `state` on approval; do not hand-write workflow IDs.
