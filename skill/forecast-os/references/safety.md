...
When a user asks for live creation or funding, use the shipped `.forecastos/config.json` public defaults unless `.forecastos/config.local.json` overrides them. Still verify approval text, wallet policy readiness, token approval when needed, and operator-provided EIP-712 signatures before submission. If creation signed fields are missing, offer concrete options such as [Privy](https://www.privy.io/ai), another EOA-compatible wallet/action tool, or the [Precog creation area](https://core.precog.markets/launchpad/) instead of asking for raw signatures in chat. Do not submit [Base MCP](https://mcp.base.org) smart-account/WebAuthn signatures for creation; the current Precog create endpoint requires a 65-byte EOA signature. Base MCP smart-wallet signatures verified through EIP-1271/ERC-6492 are accepted for funding after a prepared funding transaction is available.
